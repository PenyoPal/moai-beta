	...
	SPHINX_PATH := $(MY_MOAI_ROOT)/3rdparty

	include $(CLEAR_VARS)
	LOCAL_C_INCLUDES := $(SPHINX_PATH)/sphinxbase-0.7/include/android $(SPHINX_PATH)/sphinxbase-0.7/include
	LOCAL_CFLAGS += -DHAVE_CONFIG_H
	LOCAL_CFLAGS += -DANDROID_NDK

	LOCAL_PATH := $(SPHINX_PATH)/sphinxbase-0.7/src/libsphinxbase/util
	...
	LOCAL_PATH := $(SPHINX_PATH)/sphinxbase-0.7/src/libsphinxbase/fe
	...
	LOCAL_PATH := $(SPHINX_PATH)/sphinxbase-0.7/src/libsphinxbase/feat
	...
	LOCAL_PATH := $(SPHINX_PATH)/sphinxbase-0.7/src/libsphinxbase/lm
	...
	include $(CLEAR_VARS)
	LOCAL_C_INCLUDES := $(SPHINX_PATH)/sphinxbase-0.7/include/android $(SPHINX_PATH)/sphinxbase-0.7/include \
						$(SPHINX_PATH)/pocketsphinx-0.7/include
	LOCAL_CFLAGS += -DHAVE_CONFIG_H
	LOCAL_CFLAGS += -DANDROID_NDK

	LOCAL_PATH := $(SPHINX_PATH)/pocketsphinx-0.7/src/libpocketsphinx
	...
	LOCAL_MODULE := moaiext-sphinx

	LOCAL_C_INCLUDES 	:= $(MY_HEADER_SEARCH_PATHS)
	LOCAL_SRC_FILES += $(MY_MOAI_ROOT)/src/moaiext-sphinx/MOAISphinx.cpp
	# See http://code.google.com/p/android/issues/detail?id=9439
	PRIVATE_WHOLE_STATIC_LIBRARIES := \
		$(call static-library-path,sphinxutil) \
		$(call static-library-path,sphinxfe) \
